...
void loop()
{
  ...
  bool height[3] = {digitalRead(10), digitalRead(11), digitalRead(12)}; // 1 = I/O, 2 = Depot, 3 = Transit
  ...
  if (Serial.available() > 0) { //TEMPORARY TEST, DELETE LATER
    tempcheck = Serial.read();
  }


  while (limitcheck == 1 || stopmov == 1) { //Limit Switch reached, manually reset has to be done
    UU = digitalRead(8);
    OU = digitalRead(9);
    if (limitcheck == 1) {
  ...
  if ((tempcheck == 49) && check == 0) { //WHEN LEFT MARKER DETECTED, temp until docking detector to replace
    //motor stop and vehicle is docked
    tempcheck = 0;
    Serial.println("Vehicle Docked");
    delay(1000);
    check = 1;
    while (upcheck == 0) {
    ...
    */
    ...
    while (loadcount == 2) { //UNLOAD 1 AT BACK
      ...
      while (bottlefront == LOW && transcheck == 2) {
        ...
        for (int i = 0; bottlefront == HIGH; i++) { //sensor inconsistency
          bottlefront = digitalRead(2);
          Serial.println(i);
          delay(20);
          if (i > 5) {
    ...
    while (downcheck == 0) {
      Serial.println("Enter 3 to begin descending to transit position process");
      if (Serial.available() > 0) {
        dockinfo = Serial.read();
      }
      if (dockinfo == 51) {
